...
`marketing.studio.yaml`、`assets/marketing/brief.md`、
`assets/marketing/theme.md`、promo preview campaign 和初始 state 文件，再调用现有
launcher 做 validate 和 dry-run。studio 本身不分析图片，也不调用 vision API。
...
修改官方 theme 或沉淀 live assets 前，用 dry-run render 做人工复核。studio 不假装自动判断图片质量，也不提供用户手动添加资产的命令界面。
